...
-- SELECT COLUMN/ROW(?) FROM TABLE - this is LOOKUP
    ...
    cur_product CURSOR IS SELECT saproduct.product_code, saproduct.product_name, 
                                 saproduct.product_ID, saproduct.is_last, saproduct.price FROM SA_product_A AS saproduct
...
SELECT public.fill_table_load_log (
	...
	p_load_name => 'LOAD_TABLE_products',
	p_load_period => ?, -- ????
...
--         THEN RAISE EXCEPTION 'ERRORS found', p_e_row_count; -- логирование ошибки 
	...
	END IF;
			...
			EXCEPTION
			WHEN OTHERS THEN
				v_sa_load_id = nextval('seq_SA_load_ID'); -- что за переменная, за что отвечает?
...
SELECT fill_table_load_log (); -- или лучше CALL?

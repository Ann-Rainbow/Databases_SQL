...
-- из таблицы D_counteragent вставить в SA_invoice counteragent_ID, counteragent_code.
	...
	SELECT  COALESCE(finv.invoice_ID, '-1') AS invoice_ID, saline.invoice_code,
		    saline.amount, saline.sa_load_id, saline.load_period, saline.is_last
			--finv.invoice_ID,
...
BEGIN
	...
	IF v_line.invoice_ID = '-1' THEN 
		PERFORM public.fill_table_load_err (
			...
			p_trg_table => 'F_invoice_line_A'::character varying, 
			p_source_system_id => 0::bigint,
			p_key_value => 'Накладная не найдена', --v_debug1,
